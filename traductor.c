 ...
 * Implementa un traductor para el lenguaje Json simplificado
 * Recibe un archivo Json, retorna un archivo traducido a XML
 *	
 */
...
void errorS(){
    ...
    sprintf(msg,"Error sintactico en la linea: %d. No se esperaba %s, componente lexico: %s\n",numLinea,t.lexema,tokens[t.compLex]);
    fprintf(traducido, "%s", msg);
    e++;
}
...
int attribute_name(int synchset[]) {
   check_input(p_att_name, s_att_name, 1, 1);
   if (t.compLex == LITERAL_CADENA) {
     att_namNum++;
     fprintf(traducido, "%s", t.lexema);
     strcpy(att_namee[att_namNum], t.lexema);
   }
   match(LITERAL_CADENA);
...
void attribute(int synchset[]) {
   ...
   fprintf(traducido, "<");
   val = attribute_name(s_att_name);
   fprintf( traducido, ">");
   match(DOS_PUNTOS);  
   attribute_value(s_att_value);
   fprintf(traducido, "</");
   fprintf(traducido, "%s", att_namee[val]);
   fprintf(traducido, ">\n\t");
   att_namNum--;
...
int main() {
	if (!((fuente = fopen("fuente.txt","rt")) && (salida = fopen("output.txt", "w")) && (traducido = fopen("output.xml", "w+"))))
	{
